...
iterations = 12000        # Nombre d'itérations du modèle avec l'optimizer Adam 
...
input = np.hstack((X.flatten()[:,None], T.flatten()[:,None])) # Combine les 2 vecteurs (matrices flatten) 
...
data = dde.data.TimePDE( 
  ...
  [ic_cs, ic_cg, bc_cg, bc_dcg], # Fixe toutes les IC et BC
  ...
  anchors=input  # Points supplémentaires utilisés pour l'apprentissage
)
...
# Adam --------------------------------------------------------------------------
model.compile("adam", lr=1e-4, loss_weights = [1e0, 1e0, 1e5, 1e5, 1e5, 1e3]) # Défini l'optimiser, le learning rate et les poids accordés à chaque équations/IC/BC
...
### Post Processing ------------------------------------------------------------
output = model.predict(input) # A partir de données d'entrée, on obtient les résultats provenant du modèle après Adam
...
# LBFGS --------------------------------------------------------------------------
dde.optimizers.set_LBFGS_options(maxiter= 100, gtol= 1e-5) # Défini le nb max d'itérations et le learning rate
model.compile("L-BFGS", loss_weights = [1e2, 1e2, 1e5, 1e5, 1e4, 1e4]) # Défini l'optimiser et les poids accordés à chaque équations/IC/BC
...
### Post Processing ------------------------------------------------------------
output = model.predict(input) # A partir de données d'entrée, on obtient les résultats provenant du modèle après LBFGS
